...
cmd="$SCRIPT_CAL_MUTATIONS_STAT -k $MUTATIONS_STAT_RUNNING_KEY -w $MUTATIONS_STAT_WORKING_DIR -t $MUTATIONS_STAT_TABIX_FILE -l $MUTATIONS_STAT_LOG_FILE"
...
if [ ! -z "$MUTATIONS_STAT_COL_CONFIG" ]; then
    cmd+=" -c $MUTATIONS_STAT_COL_CONFIG"
fi
