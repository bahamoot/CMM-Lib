#!/bin/bash

export HOME_GLOB=/glob/jessada/

##********************************************* set path *****************************************************
export LIB_SCRIPT_DIR=$HOME_GLOB/lib/CMM-Lib/scripts
export SCRIPT_GEN_OAF=$LIB_SCRIPT_DIR/script_gen_oaf.sh
export SCRIPT_GEN_SA=$LIB_SCRIPT_DIR/script_gen_sa.sh
export SCRIPT_GEN_VCF_GT=$LIB_SCRIPT_DIR/script_gen_vcf_gt.sh
#export SCRIPT_GEN_GT=$LIB_SCRIPT_DIR/script_gen_gt.sh
#export SCRIPT_GEN_XLS=$LIB_SCRIPT_DIR/script_gen_xls.sh
export SCRIPT_GEN_DEFAULT_DATA=$LIB_SCRIPT_DIR/script_gen_default_data.sh
export SCRIPT_GEN_SUMMARY_XLS=$LIB_SCRIPT_DIR/script_gen_summary_xls.sh
export SCRIPT_GEN_PLINK_REPORTS=$LIB_SCRIPT_DIR/script_gen_plink_reports.sh
export SCRIPT_GEN_PLINK_REPORT=$LIB_SCRIPT_DIR/script_gen_plink_report.sh
export SCRIPT_GEN_FAMILY_XLS=$LIB_SCRIPT_DIR/script_gen_family_xls.sh
export SCRIPT_GEN_MUTATIONS_REPORTS=$LIB_SCRIPT_DIR/script_gen_mutations_reports.sh
export SCRIPT_GEN_MUTATIONS_REPORT=$LIB_SCRIPT_DIR/script_gen_mutations_report.sh
export SCRIPT_CAL_MUTATIONS_STAT=$LIB_SCRIPT_DIR/script_cal_mutations_stat.sh
export PLINK_DUMMY=$LIB_SCRIPT_DIR/plink_dummy.sh
export VCF_COL_EXIST=$LIB_SCRIPT_DIR/vcf_col_exist
export MUTS2XLS=$LIB_SCRIPT_DIR/muts2xls.py
export PLINK2XLS=$LIB_SCRIPT_DIR/plink2xls.py
#export SORT_N_AWK_CSV=$LIB_SCRIPT_DIR/sort_n_awk_csv.sh
